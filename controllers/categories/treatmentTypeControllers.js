...
exports.createTreatmentType = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    const existingTreatment = await Treatment.findOne({
      TreatmentTypeName: { $regex: new RegExp(`^${TreatmentTypeName}$`, "i") },
    });
...
exports.updatetreatmentType = async (req, res, next) => {
  try {
    const treatmentT = await Treatment.findByIdAndUpdate(
      req.params.type_id,
      req.body,
      { new: true }
    );
...
exports.deleteTreatmentType = async (req, res, next) => {
  try {
    const treatmentT = await Treatment.findOneAndDelete({
